 ...
 * columnas de contacto, centro de costo y base prendidas. El neto (`créditos − débitos`)
 * es el recaudo, y el backend lo recibe en `total`.
 *
...
export class PagoFormComponent implements OnInit, CanComponentDeactivate {
  ...
  /** Catálogo de la cuenta bancaria que recibe el recaudo. */
  ...
  ngOnInit(): void {
    ...
    if (prefetched) {
      this.applyCabecera(prefetched as PagoRead);
  ...
  protected onSubmit(): void {
    ...
    // El recaudo no puede ser negativo: los débitos (descuentos, retenciones) no
    // pueden superar a los créditos que abonan la cartera.
    if (this.totalNeto() < 0) {
      const toast = this.t().entities.pago.form.toasts.negativeTotal;
  ...
  /** Neto del recaudo (`créditos − débitos`), la misma cuenta que viaja en `total`. */
  private totalNeto(): number {
    return calcularResumenContable(this.form.controls.detalles.getRawValue()).total;
  }
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          this.applyCabecera(cabecera as PagoRead);
   ...
   */
  private applyCabecera(read: PagoRead): void {
    const value = pagoToFormValue(read);
