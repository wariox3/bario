...
@Component({
  ...
  templateUrl: './nota-ajuste-form.component.html',
  styleUrl: './nota-ajuste-form.component.scss',
})
export class NotaAjusteFormComponent implements OnInit, CanComponentDeactivate {
  ...
  protected readonly metodoPagoEndpoint = SELECT_ENDPOINTS.metodoPago;
  ...
  ngOnInit(): void {
    ...
    if (prefetched) {
      this.applyCabecera(prefetched as NotaAjusteRead);
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          this.applyCabecera(cabecera as NotaAjusteRead);
  ...
  private loadPlazoDias(): void {
    this.selectData
      .fetchOptions<PlazoPagoOption>(SELECT_ENDPOINTS.plazoPago)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (plazos) => {
