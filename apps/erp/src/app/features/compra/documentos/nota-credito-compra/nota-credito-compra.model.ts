/**
 * Contratos de datos de la **cabecera** de Nota crédito de compra.
 *
 ...
 * "NOTA CREDITO COMPRA", `documento_tipo_id = 6`). Las interfaces **extienden**
...
/** Read-model (GET `/documento/:id/`) de la cabecera de una nota crédito de compra. */
export interface NotaCreditoCompraRead extends DocumentoReadBase {
  /** Número (consecutivo) del documento que asigna el backend. */
  readonly numero: string | null;
  /** FK al documento (factura de compra) que la nota crédito referencia. */
...
/** Body (POST/PATCH) de una nota crédito de compra. */
export interface NotaCreditoCompraPayload extends DocumentoPayloadBase {
  readonly documento_referencia: number | null;
  readonly centro_costo: number | null;
  readonly comentario: string | null;
