...
@Component({
  ...
  styleUrl: './remision-form.component.scss',
})
export class RemisionFormComponent implements OnInit, CanComponentDeactivate {
  ...
  ngOnInit(): void {
    ...
    if (prefetched) {
      this.applyCabecera(prefetched as RemisionRead);
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          this.applyCabecera(cabecera as RemisionRead);
  ...
  private applyCabecera(read: RemisionRead): void {
    this.form.patchValue(remisionToFormValue(read), { emitEvent: false });
  }
