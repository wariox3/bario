 ...
 * binding (resuelto por `activeDocumentResolver` en la ruta padre) y deriva de él
 * el `documentTypeId`, las claves i18n y la ruta de la lista. El HTTP se delega en
 * `ENTITY_DATA_GATEWAY`.
 *
 * A diferencia de la factura de venta, la cabecera del pedido es **mínima** (fiel
 * al legacy): solo cliente y fecha. La **tabla de detalles** —compartida entre
...
@Component({
  ...
  templateUrl: './pedido-cliente-form.component.html',
  styleUrl: './pedido-cliente-form.component.scss',
})
export class PedidoClienteFormComponent implements OnInit, CanComponentDeactivate {
  ...
  ngOnInit(): void {
    ...
    if (prefetched) {
      this.applyCabecera(prefetched as PedidoClienteRead);
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ cabecera, lineas }) => {
          this.applyCabecera(cabecera as PedidoClienteRead);
  ...
  /** Pobla la cabecera en el form. `emitEvent: false`: no dispara efectos colaterales. */
  private applyCabecera(read: PedidoClienteRead): void {
    this.form.patchValue(pedidoClienteToFormValue(read), { emitEvent: false });
  }
