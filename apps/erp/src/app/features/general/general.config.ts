 ...
 *
 * General no tiene documentos propios: reusa los de Venta y Compra para que la
 * facturación esté a mano sin salir del módulo. Es el mismo patrón con el que
 * Venta y Compra reusan los masters de General — las páginas del documento son
 * agnósticas de módulo y derivan el suyo del `ActiveModuleStore`, así que la
 * navegación se queda dentro de General cuando se entra desde acá.
 */
export const GENERAL_CONFIG: ModuleConfig = {
  id: 'general',
  displayNameKey: 'modules.general.name',
  iconClass: 'pi pi-cog',
  documents: [FACTURA_VENTA_CONFIG, FACTURA_COMPRA_CONFIG],
};
