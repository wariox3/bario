...
import { asesorLabel, SELECT_ENDPOINTS, type ErpSelectOption } from '@reddoc/core';
 ...
 * (`{ id, nombre }`). En edición el mapper del documento siembra `nombre` con el
 * `asesor_nombre` del read; mientras el backend no lo serialice basta con el id:
 * `lib-api-select` casa la opción por `dataKey="id"` contra el catálogo que carga
 * y pinta la etiqueta desde ahí.
 */
@Component({
...
export class ErpAsesorSelectComponent implements ControlValueAccessor {
   ...
   * La misma etiqueta la usan las fichas de documento (`asesorLabel`, en core).
   */
